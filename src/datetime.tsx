...
export default class DateTime extends Component<Props> {
  onChange = (e: any) => {
    ...
    let value = moment(oldValue || undefined);
  ...
  render() {
    ...
    return (
      <View>
        {["datetime", "date"].includes(format) && (
          <Picker mode="date" value={date} onChange={this.onChange}>
            日期：{date || "请选择"}
          </Picker>
        )}
        {format === "month" && (
        ...
        {format === "datetime" && (
          <Picker mode="time" value={time} onChange={this.onChange}>
            时间：{time || "请选择"}
          </Picker>
        )}
      </View>
